...
// 		console.log("id : " + chkId);
	...
	$(function(){	
		...
		var showStrEmail = '${email2}';
		
// 		console.log(showStrEmail);
		$('#email2 option').each(function(){
			if(this.value != showStrEmail){
				$('#str_email').val(showStrEmail);
				$('#email2').val('1');
...
// 	    	 var spanId = "#reg_no_hidden";
	     ...
	     $('#bt_delete').click(function(){
			...
			var select = confirm("삭제 하시겠습니까?");
			if(select == true){
				var sabun = $('#sabun').val();
// 	    	console.log(sabun);
				$.post("/proj/delete_insa.do", {
					"sabun":sabun
				}, function(response){
						console.log('response : ' + response);
						if(response == "ok"){
							alert("삭제되었습니다.");
							window.location.href = "insa_form.do";
						}else{
							alert("삭제 실패");
							return;
						}
...
//	 			console.log("주민번호 길이 : " + $('#reg_no').val().length);
//	 			console.log('주민번호 뒤에서부터 6개 : ' + $('#reg_no').val().substr($('#reg_no').val().length-6, $('#reg_no').val().length));
//	 			console.log('치환 : ' + $('#reg_no').val().replace($('#reg_no').val().substr($('#reg_no').val().length-6, $('#reg_no').val().length), '******'));
	 		$('#reg_no').val($('#reg_no').val().replace($('#reg_no').val().substr($('#reg_no').val().length-6, $('#reg_no').val().length), '******'));
	 	}
...
// 			console.log($('#mil_yn').val())
		}
		
		$('#mil_yn').change(function(){
			var milYn = $('#mil_yn').val();
			if(milYn == 'N' || milYn == null){
				$('.mil_stat').attr("hidden", true);
			}else{
...
<body>
	...
	<article>
		<h1 style="">직원 상세 정보 수정</h1>
		<div class="bt_area">
			<input id="bt_edit" type="button" value="수정"/>
			<input id="bt_delete" type="button" value="삭제"/>
			<input type="button" value="이전" onclick="window.location.href='insa_form.do'"/>
		</div>
		<br><br>
		<div>
<!-- 			<table class="employee_table1" border="1"> -->
			<form id="insa_submit" name="insa_submit" action="/proj/insa_update.do" method="post" enctype="multipart/form-data">
				<table class="employee_table1">
					<tr>
						<td colspan="2" rowspan="6">
							<c:if test="${fn:endsWith(insa.profile,'.jpg') or fn:endsWith(insa.profile,'.png')}">
								<div id="view_area" style="width:270px; height:300px; ">
									<img id="profile_img" style="width:100%;height:100%;" src="${pageContext.request.contextPath}${insa.profile}"/>
								</div><br>
							</c:if>
							<c:if test="${!fn:endsWith(insa.profile,'.jpg') and !fn:endsWith(insa.profile,'.png')}">
								<div id="view_area" style="width:270px; height:300px; border:1px red dotted;">
							...
							<input type="text" id="img_upload_path" name="img_upload_path" value="${insa.profile}" readonly="readonly" style="background-color: lightgray; display:none;"/>
						</td>
						<td class="col_title"><span>*사번</span></td>
						<td class="input_title"><input type="text" id="sabun" name="sabun" readonly="readonly" value="${insa.sabun}"/></td>
						<td class="col_title"><span>*한글성명</span></td>
						<td class="input_title"><input type="text" id="kor_name" name="kor_name" onblur="isKorName();" value="${insa.name}"/></td>
						<td class="col_title"><span>영문성명</span></td>
						<td class="input_title" colspan="3"><input type="text" id="eng_name" name="eng_name" onblur="isEngName();" value="${insa.engName}"/></td>
					</tr>
					<tr>
						<td class="col_title"><span>*아이디</span></td>
						<td class="input_title"><input type="text" id="userid" name="userid" onblur="isId();" value="${insa.id}"/></td>
						<td class="col_title"><span>*패스워드</span></td>
						<td class="input_title">
							<input type="text" id="pwd" name="pwd"/>
							<input type="hidden" id="pwd_hidden" name="pwd_hidden" style="display:none;" value="${insa.pwd}"/>
						</td>
						<td class="col_title"><span>*패스워드</span></td>
						<td class="input_title" colspan="3">
							<input type="text" id="pwdchk" name="pwdchk"/>
							<input type="hidden" id="pwdchk_hidden" name="pwdchk_hidden" style="display:none;" value="${insa.pwd}"/>
						</td>
					</tr>
					<tr>
						<td class="col_title"><span>전화번호</span></td>
						<td class="input_title"><input type="text" id="tel" name="tel" onblur="telBlur();" value="${insa.phone}"/></td>
						<td class="col_title"><span>*핸드폰번호</span></td>
						<td class="input_title"><input type="text" id="hp" name="hp" onblur="hpBlur();" value="${insa.hp}"/></td>
						<td class="col_title"><span>주민번호</span></td>
						<td class="input_title" colspan="3">
							<input type="text" id="reg_no" name="reg_no" onblur="regNoBlur();" value="${insa.regNo}" readonly="readonly"/>
							<input type="hidden" id="reg_no_hidden" name="reg_no_hidden" value="${insa.regNo}"  style="display:none;"/>
						</td>
					</tr>
					<tr>
						<td class="col_title"><span>연령</span></td>
						<td class="input_title"><input type="text" id="age" name="age" value="${insa.years}" readonly="readonly"/></td>
						<td class="col_title"><span>*이메일</span></td>
						<td class="input_title">
							<input style="width:43%; float:left;" type="text" id="email1" name="email1" value="${email1}"/>
							...
							<select style="width:53%; float:right;" id="email2" name="email2">
								...
								<c:forEach items="${emList}" var="em">
									<option value="${em.name}"  <c:if test="${em.name eq email2}"> selected </c:if>>${em.name}</option>
								</c:forEach>
								<option value="1">직접입력</option>
							</select>
							<input type="hidden" id="email" name="email" value="${insa.email}"/>
						</td>
						...
						<td class="input_title">
							<select id="job_type" name="job_type">
								...
								<c:forEach items="${jList}" var="em">
									<option value="${em.name}" <c:if test="${em.name eq insa.jobType}"> selected </c:if>>${em.name}</option>
								</c:forEach>
							</select>
						</td>
						<td class="col_title">
						...
						<td class="input_title">
							<select id="sex" name="sex">
								...
								<c:forEach items="${sList}" var="em">
									<option value="${em.name}" <c:if test="${em.name eq insa.sex}"> selected </c:if>>${em.name}</option>
								</c:forEach>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td class="input_title"><input type="text" placeholder="우편번호" id="zip" name="zip" onblur="isZip();" value="${insa.zip}"/></td>
						<td class="input_title"><input type="button" value="주소검색" id="bt_addr" name="bt_addr" onclick="sample4_execDaumPostcode()"/></td>
						<td class="input_title"><input type="text" placeholder="주소" id="addr1" name="addr1" value="${insa.addr1}"/></td>
						<td class="input_title" colspan="4"><input type="text" placeholder="세부주소" id="addr2" name="addr2" value="${insa.addr2}"/></td>
					</tr>
					<tr>
						...
						<td class="input_title">
							<select id="pos_gbn" name="pos_gbn">
								...
								<c:forEach items="${psList}" var="em">
									<option value="${em.name}" <c:if test="${em.name eq insa.posGbnCode}"> selected </c:if>>${em.name}</option>
								</c:forEach>
							</select>
						</td>
						...
						<td class="input_title">
							<select id="dept" name="dept">
								...
								<c:forEach items="${dtList}" var="em">
									<option value="${em.name}" <c:if test="${em.name eq insa.deptCode}"> selected </c:if>>${em.name}</option>
								</c:forEach>
							</select>
						</td>
						<td class="col_title"><span>연봉(만원)</span></td>
						<td class="input_title" colspan="3">
							<input type="text" style="text-align:right;" placeholder="(만원) " id="salary" name="salary" onkeyup="inputNumberFormat(this)" value="${sal}" onload="salFormat();"/>
						</td>
					</tr>
					<tr>
						...
						<td class="input_title">
							<select id="join_gbn" name="join_gbn">
								...
								<c:forEach items="${jnList}" var="em">
									<option value="${em.name}" <c:if test="${em.name eq insa.joinGbnCode}"> selected </c:if>>${em.name}</option>
								</c:forEach>
							</select>
						</td>
						...
						<td class="input_title">
							<select id="gart_level" name="gart_level">
								...
								<c:forEach items="${gtList}" var="em">
									<option value="${em.name}" <c:if test="${em.name eq insa.gartLevel}"> selected </c:if>>${em.name}</option>
								</c:forEach>
							</select>
						</td>
						...
						<td class="input_title">
							<select id="put_yn" name="put_yn">
								...
								<c:forEach items="${ptList}" var="em">
									<option value="${em.name}" <c:if test="${em.name eq insa.putYn}"> selected </c:if>>${em.name}</option>
								</c:forEach>
							</select>
						</td>
						...
						<td class="input_title" colspan="3">
							<select id="mil_yn" name="mil_yn">
								...
								<c:forEach items="${mList}" var="em">
									<c:if test="${insa.milYn ne 'Y'}">
										<option value="${em.name}" <c:if test="${em.name eq insa.milYn}"> selected </c:if>>${em.name}</option>
									</c:if>
									<c:if test="${insa.milYn eq 'Y'}">
										<option value="${em.name}" <c:if test="${em.name eq insa.milYn}"> selected </c:if>>${em.name}</option>
									</c:if>
								</c:forEach>
							</select>
							...
							<td class="input_title">
								<select id="mil_type" name="mil_type">
									...
									<c:forEach items="${mtList}" var="em">
										<option value="${em.name}" <c:if test="${em.name eq insa.milType}"> selected </c:if>>${em.name}</option>
									</c:forEach>
								</select>
							</td>
							...
							<td class="input_title">
								<select id="mil_level" name="mil_level">
									...
									<c:forEach items="${mlList}" var="em">
										<option value="${em.name}" <c:if test="${em.name eq insa.milLevel}"> selected </c:if>>${em.name}</option>
									</c:forEach>
								</select>
							</td>
							<td class="col_title">입영일자</td>
							<td class="input_title" colspan="1">
								<input type="date" id="mil_start_date" name="mil_start_date" value="${insa.milStartDate}"/>
							</td>
							<td class="col_title">전역일자</td>
							<td class="input_title" colspan="3">
								<input type="date" id="mil_end_date" name="mil_end_date" value="${insa.milEndDate}"/>
							</td>
						</tr>
<%-- 					</c:if> --%>
					<tr>
						...
						<td class="input_title">
							<select id="kosa_reg" name="kosa_reg">
								...
								<c:forEach items="${krList}" var="em">
									<option value="${em.name}" <c:if test="${em.name eq insa.kosaRegYn}"> selected </c:if>>${em.name}</option>
								</c:forEach>
							</select>
						</td>
						...
						<td class="input_title">
							<select id="kosa_class" name="kosa_class">
								...
								<c:forEach items="${kcList}" var="em">
									<option value="${em.name}" <c:if test="${em.name eq insa.kosaClassCode}"> selected </c:if>>${em.name}</option>
								</c:forEach>
							</select>
						</td>
						<td class="col_title">*입사일자</td>
						<td class="input_title">
							<input type="date" id="join_date" name="join_date" value="${joinDate}"/>
						</td>
						<td class="col_title">퇴사일자</td>
						<td class="input_title" colspan="3">
							<input type="date" id="end_date" name="end_date" value="${retireDate}"/>
						</td>
					</tr>
					<tr>
						<td class="col_title">사업자번호</td>
						<td class="input_title">
							<input type="text" id="cmp_no" name="cmp_no" onblur="cmpNoBlur();" value="${insa.cmpRegNo}"/>
						</td>
						<td class="col_title">업체명</td>
						<td class="input_title">
							<input type="text" id="cmp_name" name="cmp_name" value="${insa.crmName}"/>
						</td>
						<td class="col_title">사업자등록증</td>
						<td class="input_title">
							<input type="text" style="background-color:lightgray;" readonly="readonly" id="cmp_img" name="cmp_img" value="${insa.cmpRegImage}"/>
						</td>
						<td colspan="4" class="bt_area2">
							<input type="button" value="미리보기" id="bt_cmp_preview" name="bt_cmp_preview" 
							class="btn default btn-lg" data-toggle="modal" data-target="#myModal1"/>
...
<!-- 							<input type="file" style="display:none;"  id="cmp_reg" name="cmp_reg"/> -->
						</td>
					</tr>
					<tr>
						...
						<td class="input_title" colspan="3">
							<textarea rows="2" cols="80" placeholder="100자 이내로 적으시오" style="resize: none;" maxlength="100"
							 id="self_intro" name="self_intro">${insa.selfIntro}</textarea>
						</td>
						<td class="col_title"><span>이력서</span></td>
						<td class="input_title">
							<input type="text" style="background-color:lightgray;" readonly="readonly" id="carrier" name="carrier" value="${insa.carrier}"/>
						</td>
						<td colspan="4" class="bt_area2">
							<input type="button" value="다운" id="bt_carrier_preview" name="bt_carrier_preview"
							class="btn default btn-lg" data-toggle="modal" data-target="#myModal2"/>
...
<!-- 							<input type="file" style="display:none;" id="carrier_reg" name="carrier_reg"/> -->
						</td>
					</tr>
				</table>
			</form>
		  ...
		  <div class="modal-dialog">
		    <div class="modal-content">
		      ...
		      <div class="modal-body">
		        <p style="text-align: center;"><img alt="이미지가 없습니다." style="width:100%;" src="${pageContext.request.contextPath}${insa.cmpRegImage}"></p>
		      </div>
		...
		<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		  <div class="modal-dialog">
		    <div class="modal-content">
		      ...
		      <div class="modal-body">
		        <p style="width:100%; text-align: center;"><img alt="이미지가 없습니다." style="width:100%;" src="${pageContext.request.contextPath}${insa.carrier}"></p>
		      </div>
		      <div class="modal-footer">
		        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
<!-- 		        <button type="button" class="btn btn-primary">Save changes</button> -->
